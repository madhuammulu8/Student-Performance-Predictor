...
Original file is located at
    https://colab.research.google.com/drive/1CjljrZz3xfCJm8bZNaTi7EdqODyM3STs
"""
...
class RandomForest:
    ...
    def __init__(self, nTrees=10, plinthRepSplit=2, supreme_depth=100, nAchv=None):
        self.nTrees = nTrees
        self.plinthRepSplit = plinthRepSplit
        self.supreme_depth = supreme_depth
        self.nAchv = nAchv
    ...
    def bootstrapRandomSample(self, R, V):
        repId = np.random.choice(R.shape[0], R.shape[0], replace=True)
        return R[repId], V[repId]

    def toughness(self, S, V):
        self.trees = []
        for _ in range(self.nTrees):
            tree = prediction(plinthRepSplit=self.plinthRepSplit,
                supreme_depth=self.supreme_depth, nAchv=self.nAchv)
            RSamp, VSamp = self.bootstrapRandomSample(S, V)
            tree.toughness(RSamp, VSamp)
            self.trees.append(tree)

    def forecast(self, S):
        for tree in self.trees:
            treePredict = np.array([tree.forecast(S)])
        treePredict = np.swapaxes(treePredict, 0, 1)
        yPredict = []
        for tree_pred in treePredict:
            yPredict.append(Counter(tree_pred).most_common(1)[0][0])
...
classification=RandomForest(supreme_depth=10)
